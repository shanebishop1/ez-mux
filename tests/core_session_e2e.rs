...
const CORE_IDS: [&str; 6] = ["E2E-01", "E2E-02", "E2E-03", "E2E-04", "E2E-05", "E2E-06"];
...
#[allow(clippy::too_many_lines)]
fn case_e2e_06(harness: &FoundationHarness) -> CaseEvidence {
    let mut assertions = Vec::new();
    let mut samples = Vec::new();
    ...
    let non_default_cwd_confirmed = pre_transition_matches_fixture
    ...
    let settle = harness
        .settle_tmux_snapshot(Duration::from_millis(50), Duration::from_secs(2))
        .unwrap_or_else(|error| panic!("E2E-06 settle evidence failed: {error}"));
